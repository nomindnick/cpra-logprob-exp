...
## 14. Phase 7 — Golden set (added 2026-09-25)

Motivation: production membership is a noisy label in both directions
(`docs/findings-run1.md` §spot check). A small, fully human-verified set with
typed strata lets us ask *where* the score changes, not just whether it ranks.

**Requests:** 24-409 (topical, well-specified) and 25-3152 (correspondent-scoped;
run with and without a CPG member roster supplied in the prompt — ablation).

**Documents:** ~250 per request, one designed pair each (no cross-pairing):

| kind | label | source |
|---|---|---|
| real_positive | 1 | pruned production, verified clearly responsive |
| real_negative | 0 | other productions, verified not responsive |
| close_call_positive | 1 | synthetic edit of a real email: marginally responsive |
| hard_negative | 0 | synthetic edit: same people/different subject; same subject/wrong date; keyword in signature or newsletter; forwarded with no content; keyword in unrelated sense |
| privileged_positive | 1 | synthetic edit: attorney–client, still responsive |
| exempt_other_positive | 1 | synthetic edit: personnel / deliberative / student records, still responsive |

Synthetic emails are **edits of real emails** (real sender, signature block,
thread shape; changed substance), generated by Opus subagents (not a Qwen — the
candidates are Qwen), each carrying its recipe and source email id. Real emails
are pre-labeled by Opus with a rationale; the human adjudicates disagreements
and a random 20%.

**Runs:** Strix Halo — qwen3.5:4b, qwen3.5:9b, qwen3.8:27b × {logprob, json}.
Laptop — qwen3.5:4b × {logprob, json}, qwen3.5:9b × logprob.

**Analyses:** score distribution by kind × model × mode; JSON self-reported
confidence vs logprob score by kind; per-kind reliability; mode agreement and
where they disagree; JSON parse-failure rate; latency by machine × model × mode
with prefill/decode split; roster ablation on 25-3152.
